...
namespace RgiSequenceFinder.TableGenerator
{
    struct GroupedEmojis
    {
        ...
        //
        // holding hands 文字みたいに skin tone が複数紛れてるものでなければ、emoji-data.json 上のインデックスは肌色で連番だと思ってよさげ。
        // (emoji-data.json の skin_variations の並びは 1F3FB-1F3FF 順で歯抜けもなさそう。)
        //
        // Gendered 系で2文字目に FE0F が付いてるのは 26F9 FE0F (person bounsing ball)だけっぽい。
        //
        // holding hands 系の絵文字も両端が (1F468 | 1F469 | 1F9D1) (1F3FB-1F3FF)+
        // (これも、emoji-data.json 内、両端の 1F3FB-1F3FF に合わせて一定順序で skin_variations 並んでる。
        // ただ、2パターンある。
        // man/woman 使ってるやつ → 元々1符号点があるものを後から 1F468, 1F469 + ZWJ にばらした
        // person (gendar neutral) 使ってるやつ → 元から 1F9D1 + ZWJ しかない)
        //
        // 1F468 👨 man
        // 1F469 👩 woman
